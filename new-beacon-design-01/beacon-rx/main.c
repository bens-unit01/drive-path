	 ...
	 if(NRF_TIMER2->EVENTS_COMPARE[0])
	    {
		    receiver_IRM_coord();
	        NRF_TIMER2->EVENTS_COMPARE[0] = 0;
//	        nrf_gpio_pin_toggle(TX_MEDIA_BOX);
...
static uint8_t data[4] = {0};
            ...
            //LOG(" %6d %6d %6d %6d  %6d %6d \n", debug[0], debug[1], debug[2], debug[3], debug[5], debug[6]);
           nrf_delay_ms(40);
		}
...
//	uint32_t err_code = sd_clock_hfclk_request();
//	LOG("Starting HFCLK err %d \n", err_code);
