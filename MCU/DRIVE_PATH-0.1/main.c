...
int main(void)
{
	...
	while (1) {
		...
		if(fNewData)
		{
			...
			// uint8_t data[3] = {0x78, fwd_bwd, lft_rgt};
			 //log(data);
                        is_driving = true;
  ...
  if (rightmotor_speed >= 800) //Right Motor Forward
    {
      ...
      nrf_pwm_set_value (1, rightmotor_speed  );
    }
  else
    { //reverse
  ...
  if (leftmotor_speed >= 800) //Left Motor Forward
    {
      ...
      nrf_pwm_set_value (0, leftmotor_speed  );
    }
  else
    { //reverse
   ...
   // uint8_t data[3] = {scale, leftmotor_speed, rightmotor_speed};
    //log(data);
...
Stop (void)
{
  ...
  if(!is_driving) nrf_gpio_pin_clear (STDBY);
...
void
pwm_timeout_handler (void * p_context)
{
  UNUSED_PARAMETER (p_context);
...
pwm_timers_init (void)
{
  ...
  // Create timers
  err_code = app_timer_create (&m_pwm_timer_id, APP_TIMER_MODE_REPEATED,
			       pwm_timeout_handler);
  APP_ERROR_CHECK(err_code);
}

void
pwm_timers_start (uint32_t time)
{
  uint32_t err_code;

  // Start application timers
  err_code = app_timer_start (m_pwm_timer_id,
			      APP_TIMER_TICKS(time, APP_TIMER_PRESCALER), NULL);
  APP_ERROR_CHECK(err_code);
