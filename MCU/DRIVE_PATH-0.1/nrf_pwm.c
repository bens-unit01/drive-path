...
void nrf_pwm_reassign_channel(uint32_t pwm_channel, uint32_t channel_io)
{
 PWM_TIMER->TASKS_STOP = 1;
 nrf_gpiote_unconfig(pwm_gpiote_channel[pwm_channel]);
 pwm_running[pwm_channel] = 0;
 pwm_io_ch[pwm_channel] = channel_io;
 nrf_gpio_cfg_output(channel_io);
 PWM_TIMER->TASKS_START = 1;
}
