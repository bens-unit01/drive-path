...
static const  char *  m_device_names[3] =
		{ "ww_beacon_01", "ww_beacon_02", "ww_beacon_03" };
static char * m_device_name = "ww_beacon_03";
...
void UART0_IRQHandler(void)
{
	...
	uint8_t data_rx = simple_uart_get();
	m_buffer[m_buffer_index] = data_rx;
	m_buffer_index++;

	if (data_rx == END_BYTE)
	{
		m_uart_available = true;
	}
	...
	if (m_uart_available)
	{
	   ...
	   if (m_buffer[0] == 'c')
       {
		 ...
		 //  simple_uart_putstring( m_device_names[m_buffer[1] - 1]);
		   m_device_name = m_device_names[m_buffer[1] - 1];//
		   m_connect = true;
