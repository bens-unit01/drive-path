...
void filter_data(uint32_t IR[], uint8_t index){
	...
	for (uint8_t i = 0; i < 4; i++)
	{
		IR[i] = ((old_ir_buffer[index][i] * 8) + (IR[i])) / 9;
        old_ir_buffer[index][i] = IR[i];
	}
...
void filter_data_2d(uint16_t IR[9][4]) {
	...
	for (uint8_t index = 0; index < 9; index++) {
		for (uint8_t i = 0; i < 4; i++) {
//			IR[index][i] = ((old_ir_buffer[index][i] * 8) + (IR[index][i])) / 9;
			IR[index][i] = ((old_ir_buffer[index][i] * 4) + (IR[index][i])) / 5;
			old_ir_buffer[index][i] = IR[index][i];
		}
	}
