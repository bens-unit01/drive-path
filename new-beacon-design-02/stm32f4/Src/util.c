...
uint32_t TIM5OF = 0;
extern TIM_HandleTypeDef htim5;  // TIM5 --> used for micros() function  
...
//	return ( TIM5OF * (htim5.Instance->ARR + 1) ) + htim5.Instance->CNT;  //number of overflows times the number of microseconds 
	//per overflow plus the current timer value in microseconds
	//
	return   (HAL_GetTick() * 1000) + (1000 - (SysTick->VAL / 168)); 	
}
