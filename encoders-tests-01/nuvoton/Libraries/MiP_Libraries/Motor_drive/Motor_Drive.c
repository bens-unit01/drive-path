...
void driveMotors(float u[])
{
	...
	//left motor
	if (u[0] < 0){
		int speed = (int)(MaxPWM * (1 + u[0]));
		PWM_Write(1, speed);			//Setting PWM Duty Cycle. PWM_Write( pin , duty cycle ).
		digitalWrite(3,2,1);											//Setting GPIO Output to High (1) or Low (0) on (Port, Pin , Output)
	   
	} 
	else{
		int speed = (int)(MaxPWM * u[0]);
		PWM_Write(1, speed);					//Setting PWM Duty Cycle. PWM_Write( pin , duty cycle ).
  ...
  // right motor
	if (u[1] > 0){
		PWM_Write(0,(int)(MaxPWM * ( 1 - u[1])));			//Setting PWM Duty Cycle. PWM_Write( pin , duty cycle ).
		digitalWrite(1,4,1);											//Setting GPIO Output to High (1) or Low (0) on (Port, Pin , Output)
	} 
	else{ 
		PWM_Write(0,(int)(-MaxPWM* (u[1])));					//Setting PWM Duty Cycle. PWM_Write( pin , duty cycle ).
	...
	if (u[2] < 0){
		PWM_Write(3,(int)(-MaxPWM*u[2]));					//Setting PWM Duty Cycle. PWM_Write( pin , duty cycle ).	
	...
	//printf("driveMotors u[0] : %f u[1] : %f \n", u[0], u[1]);
	//Right Tread Motor
	if (u[0] > 0){
	...
	if (u[1] > 0){
		PWM_Write(3,(int)(MaxPWM*u[1]));					//Setting PWM Duty Cycle. PWM_Write( pin , duty cycle ).
		digitalWrite(0,2,1);											//Setting GPIO Output to High (1) or Low (0) on (Port, Pin , Output)
											//Setting GPIO Output to High (1) or Low (0) on (Port, Pin , Output)
	}
	else{
		PWM_Write(3,(int)(-MaxPWM*u[1]));					//Setting PWM Duty Cycle. PWM_Write( pin , duty cycle ).
		digitalWrite(0,2,0);											//Setting GPIO Output to High (1) or Low (0) on (Port, Pin , Output)
	...
	//Left Tread Motor
	if (u[2] > 0){
		int speed = (int)(MaxPWM * (u[2]));
		PWM_Write(1, 0);			//Setting PWM Duty Cycle. PWM_Write( pin , duty cycle ).
		digitalWrite(3,2, 0);											//Setting GPIO Output to High (1) or Low (0) on (Port, Pin , Output)
